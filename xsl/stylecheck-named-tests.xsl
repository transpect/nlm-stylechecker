...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"   
      ...
      <xsl:if
         test="
            related-article[not(@related-article-type = 'republished-article') and
               ...
               not(@related-article-type = 'repub-note')] and 
         ...
         <xsl:when test="parent::fig">
            ...
            <xsl:if test="media">
               <xsl:call-template name="check-media-graphic-in-alt">
                  <xsl:with-param name="nodes" select="media"/>
               </xsl:call-template>
            </xsl:if>
            <xsl:if test="inline-media">
               <xsl:call-template name="check-media-graphic-in-alt">
                  <xsl:with-param name="nodes" select="inline-media"/>
               </xsl:call-template>
            </xsl:if>
            <xsl:if test="graphic">
               <xsl:call-template name="check-media-graphic-in-alt">
   ...
   <!-- *********************************************************** -->
   <!-- Template: check-media-graphic-in-alt
		$nodes = elements with attribute to check
		
		Checks that at least one of the $nodes has
                  an xlink:href that does not point to a URI.
                  PMC-41172
	-->
   <!-- *********************************************************** -->
   <xsl:template name="check-media-graphic-in-alt">
      <xsl:param name="nodes"/>
      <xsl:if test="$nodes[starts-with(@xlink:href,'http')]">
            <xsl:choose>
                <!-- this is ok --> 
               <xsl:when test="$nodes[not(starts-with(@xlink:href, 'http'))]">
               </xsl:when>
               <xsl:otherwise>
                  <xsl:call-template name="make-error">
                     <xsl:with-param name="error-type">alternatives content
                     ...
                     <xsl:with-param name="description">
                        <xsl:text>&lt;alternatives&gt; in &lt;</xsl:text>
                        <xsl:value-of select="name(parent::node())"/>
                        <xsl:text>&gt; must have at least one local file.</xsl:text>
                     </xsl:with-param>
                     <xsl:with-param name="tg-target"
                        select="'tags.html#el-alternatives'"/>
                  </xsl:call-template>
   ...
   <xsl:template name="article-type-content-check">
      ...
      <xsl:choose>
         ...
         <!-- A response or sub-article must not contain a correction/retraction/expression-of-concern
            unless the parent article is of the same artice-type and a different language -->
         <xsl:when test="name($context)='sub-article' 
            ...
            <xsl:choose>
               <xsl:when test="$normalized = 'correction' 
                  ...
                  <xsl:choose>
                     <xsl:when test="name($context)='sub-article'">
                        <!-- check if parent is of the same article-type but a different language -->
                        <xsl:choose>
                           <xsl:when test="$normalized = /article/@article-type
                                 and $context/@xml:lang != /article/@xml:lang">
                              <!-- this is okay -->
                           </xsl:when>
                           <xsl:otherwise>
                              <xsl:call-template name="make-error">
                                 ...
                                 <xsl:with-param name="description">
                                    ...
                                    <xsl:text> should not have an article-type value of '</xsl:text>
                                    <xsl:value-of select="$normalized"/><xsl:text>'</xsl:text>.</xsl:with-param>
      ...
      <xsl:variable name="has-error">
         <xsl:choose>
            <xsl:when test="$context[self::ack]">
               <xsl:choose>
                  <xsl:when
                     test="$context[parent::sec/ancestor::body or parent::body] and $stream != 'book'">
  ...
  -->
   ...
   <!-- *********************************************************** -->
   <!-- Template: custom-type-check

        1) when @custom-meta is present, the standard type attribute 
           (@pub-id-type, @fn-type, etc.) must be present and have a 
           value of "custom"
        2) when the standard type attribut (@ref-type, @pub-id-type, etc.) 
           is present and has a value of "custom", @custom-type must be
           present and non-empty
     -->
   <!-- *********************************************************** -->
   <xsl:template name="custom-type-check">
      <xsl:param name="standard-type"/>
      <xsl:param name="attribute-name"/>
      <xsl:if test="@custom-type and ($standard-type != 'custom' or not($standard-type))">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'custom-type check'"/>
            <xsl:with-param name="description">
               <xsl:text>When @custom-type is present, @</xsl:text><xsl:value-of select="$attribute-name"/><xsl:text> must be present and have a value of "custom" </xsl:text>
               <xsl:call-template name="get-context"/>
            </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
      <xsl:if test="$standard-type = 'custom' and (@custom-type = '' or not(@custom-type))">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type" select="'custom-type check'"/>
            <xsl:with-param name="description">
               <xsl:text>When the value of @</xsl:text><xsl:value-of select="$attribute-name"/><xsl:text> is "custom", @custom-type must be present and non-empty </xsl:text>
               <xsl:call-template name="get-context"/>
            </xsl:with-param>
         </xsl:call-template>
      </xsl:if>
   ...
   <xsl:template name="doi-format-test">
      ...
      <xsl:choose>
         ...
         <!-- Must not have extra dot after '10.' -->
         <xsl:when test="starts-with($normalized-doi, '10..')">
            <xsl:text>false</xsl:text>
         </xsl:when>
   ...
   <!-- *********************************************************** -->
   <!-- Template: dup-pub-id-type-check
        1) @pub-id-type must be unique, except for @pub-id-type="publisher-id",
                   @pub-id-type="publisher-manuscript", @pub-id-type="other", and
                   @pub-id-type="custom" - multiple instances of
                   those are allowed
                -->
   <!-- *********************************************************** -->
   <xsl:template name="dup-pub-id-type-check">
      <xsl:variable name="type" select="@pub-id-type"/>
      <xsl:choose>
         <xsl:when test="$type = 'other' or $type = 'publisher-id' or $type = 'publisher-manuscript' or $type = 'custom'"/>
         <xsl:when test="@pub-id-type = preceding-sibling::article-id/@pub-id-type">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">duplicate pub-id-type check</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>metadata should contain only one &lt;article-id&gt; with @pub-id-type="</xsl:text>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:variable name="is-named-season-range-with-autumn">
         	...
         	<xsl:variable name="has-ok-iso-8601-date">
         		<xsl:variable name="date" select="parent::pub-date/@iso-8601-date"/>
         		...
         		<xsl:choose>
         			<xsl:when
         				...
         				string-length($day) = 2 and number($day) and
         				string-length($mo) = 2 and number($mo) and
         				string-length($yr) = 4 and number($yr)
         				"
         				>true</xsl:when>
            ...
            <xsl:choose>
               ...
               <xsl:when test="$is-long-name-abbreviated-range = 'true'">
            	...
            	<!-- allow deviations when @iso-8601-date is present and properly formatted -->
            	<xsl:when test="$has-ok-iso-8601-date = 'true'">
            		<xsl:text>true</xsl:text>
            	</xsl:when>
   ...
   <xsl:template name="kwd-group-check">
	...
	<!-- *********************************************************** -->
	<!-- Template: license-cc-url-check
		See PMC-43981: license URL must be valid
	-->
	<!-- *********************************************************** -->
	
	<xsl:template name="license-cc-url-check">
		<xsl:param name="href"/>
		<xsl:variable name="href-ns" select="normalize-space($href)"/>
		<xsl:if test="contains($href-ns, 'creative') and contains($href-ns, 'commons') and not(contains($href-ns, 'us.sagepub.com'))">
			<!-- see PMC-46551 for definition of "CC" url -->
			<xsl:variable name="href-standardized">
				<xsl:call-template name="standardize-license-url">
					<xsl:with-param name="href" select="$href-ns"/>
				</xsl:call-template>
			</xsl:variable>
			<!-- list of OK URLs -->
			<xsl:variable name="ok-urls">
				<xsl:call-template name="canonical-cc-license-urls"/>
			</xsl:variable>
			<xsl:if test="not(contains($ok-urls, concat(' ', $href-standardized, ' ')))">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type">license cc url check</xsl:with-param>
					<xsl:with-param name="description">
						<xsl:text>The Creative Commons license URL does not point to a standard machine-readable Creative Commons license, </xsl:text>
						<xsl:text>please check for errors in the URL and/or extra terms (such as legalcode or deed.en) </xsl:text>
						<xsl:text>after the license version number. URL: </xsl:text>
						<xsl:value-of select="$href"/>
					</xsl:with-param>
					<xsl:with-param name="tg-target" select="'tags.html#el-license'"/>
				</xsl:call-template>
			</xsl:if>
			<!-- issue warning on 'licences' -->
			<xsl:if test="contains($href, '/licences/')">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type">license cc url check</xsl:with-param>
					<xsl:with-param name="description"
						select="concat('The Creative Commons license URL contains variant spelling (licences): ', $href)"/>
					<xsl:with-param name="tg-target">tags.html#el-license</xsl:with-param>
					<xsl:with-param name="class">warning</xsl:with-param>
				</xsl:call-template>
			</xsl:if>
		</xsl:if>
	...
	<xsl:template name="standardize-license-url">
		<xsl:param name="href"/>
		<xsl:variable name="href-ns" select="normalize-space($href)"/>
		<!-- normalize minor license URL deviations away: protocol, trailing slash legit spelling -->
		<xsl:variable name="href-prefix-trimmed">
			<xsl:choose>
				<xsl:when
					test="starts-with($href-ns, 'http://www.') or starts-with($href-ns, 'https://www.') or starts-with($href-ns, 'www.')">
					<xsl:value-of select="substring-after($href-ns, 'www.')"/>
				</xsl:when>
				<xsl:when
					test="starts-with($href-ns, 'http://') or starts-with($href-ns, 'https://')">
					<xsl:value-of select="substring-after($href-ns, '://')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="($href-ns)"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- remove a trailing slash -->
		<xsl:variable name="href-slash-trimmed">
			<xsl:choose>
				<xsl:when
					test="substring($href-prefix-trimmed, string-length($href-prefix-trimmed), 1) = '/'">
					<xsl:value-of
						select="substring($href-prefix-trimmed, 1, string-length($href-prefix-trimmed) - 1)"
					/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$href-prefix-trimmed"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- standardize spelling -->
		<xsl:choose>
			<xsl:when test="contains($href-slash-trimmed, '/licences/')">
				<xsl:value-of
					select="
					concat(
					substring-before($href-slash-trimmed, '/licences/'),
					'/licenses/',
					substring-after($href-slash-trimmed, '/licences/')
					)"
				/>
			</xsl:when>
			<xsl:when test="$href-slash-trimmed = 'creativecommons.org/share-your-work/public-domain/cc0'">
				<!-- map per PMC-43981 -->
				<xsl:text>creativecommons.org/publicdomain/zero/1.0</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$href-slash-trimmed"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
   ...
   <!-- Template: license-ext-link-content-check
		See PMC-15983, deprecated see PMC-43981
   ...
   <!-- *********************************************************** -->
	<xsl:template name="license-ext-link-content-check">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="normalized-href">
					<xsl:call-template name="standardize-license-url">
						<xsl:with-param name="href" select="@xlink:href"/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:variable name="normalized-linktext">
					<xsl:call-template name="standardize-license-url">
						<xsl:with-param name="href" select="."/>
					</xsl:call-template>
				</xsl:variable>
				<xsl:if test="string($normalized-href) != string($normalized-linktext)">
					<xsl:call-template name="make-error">
						<xsl:with-param name="error-type">license/ext-link check</xsl:with-param>
						<xsl:with-param name="description">
							<xsl:text>URIs in ext-link element content and @xlink:href must be identical but are not.  Content URI: [</xsl:text>
							<xsl:value-of select="$str"/>
							<xsl:text>] @xlink:href URI: [</xsl:text>
   ...
   <!-- Template: license-integrity-check
		See PMC-15983
		1) If license has @xlink:href and ext-link, the xlink:href values must match
		2) If license has ali:license_ref and ext-link, the xlink:href values must match
		3) If license has multiple ext-links, the xlink:href values must match
		4) A license may not have more than one ali:license_ref with a Creative Commons URLs
	-->
   <!-- *********************************************************** -->
   <xsl:template name="license-integrity-check">
      <xsl:param name="context" select="."/>
   	<xsl:variable name="cc-ext-links" select="$context//ext-link[contains(@xlink:href, 'creativecommons.org/')]"/>
   	<xsl:variable name="cc-refs" select="$context/ali:license_ref[contains(., 'creativecommons.org/')]"/>
   	<!-- compare license/@xlink:href to ext-link -->
      <xsl:if
      	test="$context/@xlink:href[contains(., 'creativecommons.org/')] and $cc-ext-links">
      	<xsl:call-template name="license-compare-links">
      		<xsl:with-param name="license-href" select="$context/@xlink:href"/>
      		<xsl:with-param name="ext-link-href" select="$cc-ext-links[1]/@xlink:href"/>
      		<xsl:with-param name="parent">license</xsl:with-param>
      	</xsl:call-template>
      </xsl:if>
   	<!-- compare ali:license_ref to ext-link -->
   	<xsl:if
   		test="$cc-refs and $cc-ext-links">
   		<xsl:call-template name="license-compare-links">
   			<xsl:with-param name="license-href" select="$cc-refs[1]"/>
   			<xsl:with-param name="ext-link-href" select="$cc-ext-links[1]/@xlink:href"/>
   			<xsl:with-param name="parent">ali:license_ref</xsl:with-param>
   		</xsl:call-template>
   	</xsl:if>
   	<!-- compare external links -->
      ...
      <!-- check ali:license_ref -->
   	<xsl:if test="count(ali:license_ref[contains(., 'creativecommons.org')]) > 1">
   		<xsl:call-template name="make-error">
   			<xsl:with-param name="error-type">cc license_ref check</xsl:with-param>
   			<xsl:with-param name="description">
   				<xsl:text>A license may not have more than one ali:license_ref elements with a Creative Commons URL.</xsl:text>
   			</xsl:with-param>
   			<xsl:with-param name="tg-target" select="'dobs.html#dob-license'"/>
   		</xsl:call-template>
   	</xsl:if>
   ...
   <xsl:template name="license-ext-link-recursion">
	...
	<xsl:template name="license-compare-links">
		<xsl:param name="license-href"/>
		<xsl:param name="ext-link-href"/>
		<xsl:param name="parent"/>
		<!-- normalize the links before comparing: this takes care 
			of this like trailing slashes or https vs http -->
		<xsl:variable name="normalized-license-href">
			<xsl:call-template name="standardize-license-url">
				<xsl:with-param name="href" select="$license-href"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="normalized-ext-link-href">
			<xsl:call-template name="standardize-license-url">
				<xsl:with-param name="href" select="$ext-link-href"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="$license-href = '' or $ext-link-href = '' ">
				<!-- something went wrong, template should not have been called -->
			</xsl:when>
			<xsl:when test="$normalized-ext-link-href = $normalized-license-href">
				<!-- we're good! -->
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="make-error">
					...
					<xsl:with-param name="description">
						<xsl:choose>
							<xsl:when test="$parent = 'license'">
								<xsl:text>URI values in @xlink:href on &lt;license&gt; and in an &lt;ext-link&gt; descendant of that &lt;license&gt; must be identical but are not. license/@xlink:href URI: [</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text>URI values in &lt;ali:license_ref&gt; and in an &lt;ext-link&gt; descendant of that &lt;license&gt; must be identical but are not. ali:license_ref URI: [</xsl:text>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:value-of select="$license-href"/>
						<xsl:text>] ext-link descendant URI: [</xsl:text>
						<xsl:value-of select="$ext-link-href"/>
						<xsl:text>]. </xsl:text>
                                                <xsl:call-template name="get-context"/>
					</xsl:with-param>
					<xsl:with-param name="tg-target"
   ...
   <xsl:template name="list-type-check">
	...
	<!-- Template: list-continued-check
        ...
        1) The continued-from attribute may not point to the parent list.
        2) The continued-from attribute must point to another list
        3) The continued-from attribute may not point to a list that continues the currrent list 
	...
	<xsl:template name="list-continued-check">
		...
		<xsl:choose>
			<!-- no continued from attribute -->
			<xsl:when test="$lc = ''"/>
			<!-- continued-from may not point to the parent list -->
			...
			<!-- continued-from attribute must point to another list  -->
			<xsl:when test="local-name(key('el-by-id', $lc)) != 'list' ">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'continued list check'"/>
					<xsl:with-param name="description">
						<xsl:text>The continued-from attribute must reference a list, not a </xsl:text>
						<xsl:value-of select="name(key('el-by-id', $lc))"/>
						<xsl:text>.</xsl:text>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
			<!-- continued-from attribute may not point to a list that continues the currrent list -->
			<!-- 
				<list list-type="order" id="L20" continued-from="L21">
 				<list list-type="order" id="L21" continued-from="L20">
			-->
			<xsl:when test="string(key('el-by-id', $lc)/@continued-from) = string($context/@id)">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'continued list check'"/>
					<xsl:with-param name="description">
						<xsl:text>The continued-from attribute may not reference a list that continues the current list.</xsl:text>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
		</xsl:choose>
...
<!-- only mml:mtext and mml:mn are sent to this test -->
   ...
   <!-- *********************************************************** -->
   <!-- Template: mathml-ancestor-el-check 
      
      Make sure that context element exists only within specified ancestor element
      
   -->
   <!-- Specifically added for ND-2292 and maligngroup element. Needs specific parents and ancestor of <mtd> -->
   <!-- *********************************************************** -->
   <xsl:template name="mathml-ancestor-el-check">
      <xsl:param name="expected-ancestor"/>
      
      <xsl:if test="not(ancestor::*[local-name() = $expected-ancestor])">
         <xsl:call-template name="make-error">
            <xsl:with-param name="error-type"
               select="'mathml ancestor element check'"/>
            <xsl:with-param name="description">
               <xsl:text>mathml element </xsl:text>
               <xsl:value-of select="local-name(.)"/>
               <xsl:text> must be a descendant of the </xsl:text>
               <xsl:value-of select="$expected-ancestor"/>
               <xsl:text> element</xsl:text>
            </xsl:with-param>
         </xsl:call-template>
      </xsl:if>
   ...
   <!-- Template: pmid-check
        ...
        The value of pub-id[@pub-id-type='pmid'] must not begin with a
	zero and must not contain letters.
   -->
   <!-- *********************************************************** -->
   <xsl:template name="pmid-check">
     <xsl:param name="value"/>
     <xsl:variable name="pmid" select="normalize-space($value)"/>
     <xsl:if test="contains(substring($pmid, 1, 1), '0')">
       <xsl:call-template name="make-error">
	 <xsl:with-param name="error-type" select="'PMID format check'"/>
	 <xsl:with-param name="description">
	   <xsl:text>PMID must not begin with a zero</xsl:text>
	 </xsl:with-param>
       </xsl:call-template>
     </xsl:if>
     <xsl:if test="not(string-length(translate($pmid,'0123456789', '')) = 0)">
       <xsl:call-template name="make-error">
	 <xsl:with-param name="error-type" select="'PMID format check'"/>
	 <xsl:with-param name="description">
	   <xsl:text>PMID must contain only numbers</xsl:text>
	 </xsl:with-param>
       </xsl:call-template>
     </xsl:if>
   </xsl:template>
   ...
   <xsl:template name="related-article-check">
      ...
      <xsl:variable name="secret-ok-list"
         select="
            concat(
            'peer-review ',
      ...
      <xsl:variable name="rat-ok">
         <xsl:call-template name="is-in-list">
            <xsl:with-param name="list" select="concat(translate($ok-list, ',', ''), translate($secret-ok-list, ',', ''))"/>
   ...
   <xsl:template name="ms-related-article-check">
      ...
      <xsl:if
         test="
            ...
            @related-article-type != 'reprinted-article' and
            @related-article-type != 'repub-note'">
         <xsl:call-template name="make-error">
      ...
      <xsl:if
         test="not(@ext-link-type = 'pmcid') and not(@ext-link-type = 'pmid') and not(@ext-link-type = 'doi')">
         <xsl:call-template name="make-error">
      ...
      <xsl:if test="not(number(@xlink:href)) and not(starts-with(@xlink:href, '10.'))">
         <xsl:call-template name="make-error">
   ...
   <xsl:template name="related-article-to-article-type-attribute">
      ...
      <xsl:choose>
      	<!-- preprint -->
      	<xsl:when test="$context-type-att = 'preprint'">
      		<!-- anything goes -->
      	</xsl:when>
      	<!-- published-article -->
      	<xsl:when test="$context-type-att = 'published-article'">
      		
      		<xsl:choose>
      			<!-- This is correct -->
      			<xsl:when test="$context/ancestor::article-meta/descendant::article-version[@article-version-type = 'status' and . = 'preprint']"/>
      			...
      			<xsl:otherwise>
      				...
      				<xsl:call-template name="make-error">
      					...
      					<xsl:with-param name="description"
      						select="
      						concat(
      						'Either the ', $context-att-name, ' attribute in ',
      						$context-name, ' is incorrect or an &lt;article-version> element is missing: The ', $context-att-name,
      						' suggests that this article is a preprint. ', 
      						'Every preprint must have an &lt;article-version> with the @article-version-type set to ',
      						$squot, 'status', $squot, ' and the value set to ', $squot, 'preprint', $squot, '. ',
      						'However, the article-version element is not present in this article.')"
      					/>
      				</xsl:call-template>
      			</xsl:otherwise>
      		</xsl:choose>
         ...
         <!-- Now, check the types, stop if there is no article-type attribute -->
         ...
         <xsl:otherwise>
            ...
            <xsl:if test="$is-valid-season = 'false'">
               <xsl:call-template name="make-error">
                  ...
                  <xsl:with-param name="description">
                     <xsl:text>season should only contain a named season (Spring, Summer, Fall, Autumn, Winter), </xsl:text>
                  	<xsl:text>a month range (Jan-Feb or January-February), or a named season range (Spring-Summer). Current content is: </xsl:text>
                     <xsl:value-of select="$context"/>
                  </xsl:with-param>
                  <xsl:with-param name="tg-target"
...
<xsl:with-param name="error-type">string-date check</xsl:with-param>
   ...
   <xsl:template name="table-fn-check">
	...
	<!-- *********************************************************** -->
	<!-- Template: table-span-check 
   
        table spanning attributes must contain an integer -->
	<!-- *********************************************************** -->
	<xsl:template name="table-span-check">
		<xsl:param name="context" select="."/>
		<xsl:for-each select="$context/@rowspan | $context/@colspan | $context/@span ">
			<xsl:if test=". = '' or translate(., '0123456789', '') != '' ">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type"
						select="'table span checking'"/>					
					<xsl:with-param name="description">
						<xsl:text>A </xsl:text>
						<xsl:value-of select="name()"/>
						<xsl:text> attribute must contain a positive integer, not "</xsl:text>
						<xsl:value-of select="."/>
						<xsl:text>"</xsl:text>
					</xsl:with-param>
				</xsl:call-template>
   ...
   <xsl:template name="xlink-attribute-check">
	...
	<!-- *********************************************************** -->
	<!-- Template: xlink-ns-check 
   
        Makes sure that xmlns:xlink always points to http://www.w3.org/1999/xlink
        Note: we've seen articles
        with bad xmlns:xlink declararion, but not xlink:* attribute
        <ext-link ext-link-type="uri" xmlns:xlink="http://adni.loni.usc.edu">
     -->
	<!-- *********************************************************** -->
	<xsl:template name="xlink-ns-check">
		<xsl:param name="context" select="."/>
		<xsl:for-each select="$context/namespace::xlink">
			<xsl:if test=". != 'http://www.w3.org/1999/xlink'">
				<xsl:call-template name="make-error">
					<xsl:with-param name="error-type" select="'xlink namespace check'"/>
					<xsl:with-param name="class">warning</xsl:with-param>
					<xsl:with-param name="description">
						<xsl:text>The namespace URI for XLink must be "http://www.w3.org/1999/xlink", not "</xsl:text>
						<xsl:value-of select="."/>
						<xsl:text>"</xsl:text>
					</xsl:with-param>
				</xsl:call-template>
   ...
   <xsl:template name="xref-check">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$context[@rid and not(@ref-type)]">
            <xsl:call-template name="make-error">
               <xsl:with-param name="error-type">xref attribute check</xsl:with-param>
               <xsl:with-param name="class">warning</xsl:with-param>
               <xsl:with-param name="description">
                  <xsl:text>xref element must have @ref-type.</xsl:text>
               </xsl:with-param>
            </xsl:call-template>
         </xsl:when>
         ...
         <xsl:when test="$context/@ref-type = 'supplementary-material'">
		  ...
		  or self::inline-supplementary-material
                     ...
                     <xsl:call-template name="write-xref-error-description">
                        ...
                        <xsl:with-param name="expected-target"
                           select="
                              ...
                              'an inline-supplementary-material element, ',
